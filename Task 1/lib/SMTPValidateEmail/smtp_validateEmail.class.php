	  ...
	  if ($this->sock) {
	   ...
	   if($code != '220') {
	      ...
	      foreach($users as $user) {
                // Ruben 2014-04-18: this error means the Mail Transfer Agent did not accept the request,
                // so we could assume the email address is valid since it might just be offline
                $results[$user.'@'.$domain] = $this->assumeOk;
		}
		continue;
	   }
	   ...
	   foreach($users as $user) {
		   ...
		   $code = isset($matches[1]) ? $matches[1] : '';
                   ...
                   // Ruben 2014-04-18: empty response fix
		   if (($this->assumeOk && ($reply == '' ||     // rejected by mail server
                       strripos($reply, 'Protocol error'))) ||  // buggy mail server SMTP
                       $code == '250' ||                        // you received 250 so the email address was accepted
                       $code == '451' || $code == '452') {      // you received 451 so the email address was greylisted (or some temporary error occured on the MTA) - so assume is ok
